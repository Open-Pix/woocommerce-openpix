...
require_once 'config/config-prod-beta.php';

add_action('admin_footer', 'embedWebhookConfigButton_prod');

function embedWebhookConfigButton_prod()
{
    ?>
...
add_action('wp_ajax_openpix_configure_webhook', [
    'WC_OpenPix_Pix_Prod_Gateway',
    'openpix_configure_webhook',
]);
function wc_openpix_assets_prod_url()
{
    return plugin_dir_url(dirname(__FILE__)) . 'assets/';
}

class WC_OpenPix_Pix_Prod_Gateway extends WC_Payment_Gateway
{
    public function __construct()
    {
        $this->id = 'woocommerce_openpix_pix_prod';
        $this->method_title = __('OpenPix [PROD]', 'woocommerce-openpix');
        $this->method_description = __(
            'WooCommerce OpenPix Payment Gateway [PROD]',
            'woocommerce-openpix'
        );
    ...
    {
        if (OpenPixConfig_Prod::getEnv() === 'development') {
            return 'http://localhost:6688/main.js';
        }

        if (OpenPixConfig_Prod::getEnv() === 'staging') {
            return plugins_url(
    ...
    {
        $webhookUrl = OpenPixConfig_Prod::getWebhookUrl();
        $this->form_fields = [
            'enabled' => [
            ...
            'title' => [
                ...
                'desc_tip' => true,
                'default' => __('Pay with Pix [ PROD ]', 'woocommerce-openpix'),
            ],
            'description' => [
    ...
    public function process_payment($order_id)
    {
        ...
        $url = OpenPixConfig_Prod::getApiUrl() . '/api/openpix/v1/charge';
    ...
    public static function openpix_configure_webhook()
    {
        ...
        $url = OpenPixConfig_Prod::getApiUrl() . '/api/openpix/v1/webhook';
    ...
    public function thankyou_page($order_id)
    {
        ...
        $environment = OpenPixConfig_Prod::getEnv();
        $queryString = "appID={$this->appID}&correlationID={$correlationID}&node=openpix-order";
        $pluginUrl = OpenPixConfig_Prod::getPluginUrl();
